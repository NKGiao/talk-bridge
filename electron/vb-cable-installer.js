...
async function waitForInstallation(maxAttempts = 30, interval = 2000) {
  console.log('[GM MeetMind] [VB-CABLE Installer] Waiting for installation to complete...');
  ...
  for (let i = 0; i < maxAttempts; i++) {
    ...
    if (await isVBCableInstalled()) {
      console.log('[GM MeetMind] [VB-CABLE Installer] Installation detected successfully');
      return true;
    }

    if (i % 5 === 0 && i > 0) {
      console.log(`[GM MeetMind] [VB-CABLE Installer] Still waiting... (${i * interval / 1000}s elapsed)`);
    }
  }
...
async function cleanupTempFiles(tempDir) {
  try {
    ...
    console.log('[GM MeetMind] [VB-CABLE Installer] Temporary files cleaned up');
  } catch (cleanupError) {
    console.warn('[GM MeetMind] [VB-CABLE Installer] Could not clean up temp files:', cleanupError);
  }
}
...
async function installVBCable(silent = false) {
  try {
    console.log('[GM MeetMind] [VB-CABLE Installer] Starting VB-CABLE installation process...');

    // Check if already installed using the shared detection function
    const { isVBCableInstalled } = require('./windows-audio-utils');
    if (await isVBCableInstalled()) {
      console.log('[GM MeetMind] [VB-CABLE Installer] VB-CABLE is already installed');
      return true;
    }

    // Note: We no longer check for admin privileges here
    // Windows UAC will automatically prompt for elevation when needed
    console.log('[GM MeetMind] [VB-CABLE Installer] Preparing installation (Windows will handle UAC if needed)...');
    ...
    console.log('[GM MeetMind] [VB-CABLE Installer] Downloading VB-CABLE...');
    await downloadFile(vbCableUrl, zipPath);

    // Extract the ZIP file
    console.log('[GM MeetMind] [VB-CABLE Installer] Extracting VB-CABLE...');
    ...
    try {
      ...
      console.error('[GM MeetMind] [VB-CABLE Installer] Installer not found at:', installerPath);
      return false;
    }

    // Run the installer - Windows will automatically show UAC prompt if needed
    console.log('[GM MeetMind] [VB-CABLE Installer] Launching installer:', installerPath);

    const { shell, dialog } = require('electron');

    if (silent) {
      // Silent installation using PowerShell to trigger UAC
      console.log('[GM MeetMind] [VB-CABLE Installer] Attempting silent installation...');
      ...
      try {
        ...
        console.log('[GM MeetMind] [VB-CABLE Installer] Installation process completed');

        // Check if installation was successful
        const installed = await waitForInstallation(5, 2000);
        if (installed) {
          console.log('[GM MeetMind] [VB-CABLE Installer] VB-CABLE installed successfully');
          cleanupTempFiles(tempDir);
          return true;
        }
      } catch (error) {
        console.error('[GM MeetMind] [VB-CABLE Installer] Silent installation failed:', error);
        // Fall back to interactive installation
        console.log('[GM MeetMind] [VB-CABLE Installer] Falling back to interactive installation...');
      }
    }

    // Interactive installation - trigger UAC properly
    console.log('[GM MeetMind] [VB-CABLE Installer] Launching installer with UAC elevation...');
    ...
    try {
      ...
      console.log('[GM MeetMind] [VB-CABLE Installer] Installer launched with elevation request');
    } catch (error) {
      if (error.message && error.message.includes('canceled')) {
        console.log('[GM MeetMind] [VB-CABLE Installer] User cancelled UAC prompt');
        cleanupTempFiles(tempDir);
        return false;
      }

      // If PowerShell method fails, try VBScript alternative
      console.warn('[GM MeetMind] [VB-CABLE Installer] PowerShell method failed, trying VBScript alternative...');
      ...
      try {
        ...
        console.log('[GM MeetMind] [VB-CABLE Installer] Installer launched via VBScript with elevation');
      } catch (vbsError) {
        console.error('[GM MeetMind] [VB-CABLE Installer] All elevation methods failed:', vbsError);

        // Last resort: just try to run it normally and hope for the best
        await shell.openPath(installerPath);
        console.log('[GM MeetMind] [VB-CABLE Installer] Fallback: Opened installer without explicit elevation');
      }
    }
    ...
    if (guidanceResult.response === 1) {
      console.log('[GM MeetMind] [VB-CABLE Installer] User cancelled installation');
      cleanupTempFiles(tempDir);
      return false;
    }
    ...
    if (installed) {
      ...
      console.error('[GM MeetMind] [VB-CABLE Installer] VB-CABLE installation failed or was cancelled');
      cleanupTempFiles(tempDir);
      return false;
    }
  } catch (error) {
    console.error('[GM MeetMind] [VB-CABLE Installer] Installation failed:', error);
    return false;
  }
}
...
async function ensureVBCableInstalled() {
  try {
    ...
    console.error('[GM MeetMind] [VB-CABLE Installer] Error in installation flow:', error);
    return false;
  }
}

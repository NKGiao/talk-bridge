...
(function() {
  console.info('[GM MeetMind] [VirtualMic] Virtual Microphone script loaded');
  ...
  function initializeVirtualMic() {
    if (trackGenerator && audioWriter && isWriterValid()) {
      console.debug('[GM MeetMind] [VirtualMic] Virtual microphone already initialized');
      if (!virtualStream.active) {
        console.debug('[GM MeetMind] [VirtualMic] Virtual microphone is not active, reinitializing');
        cleanup();
      } else {
        console.debug('[GM MeetMind] [VirtualMic] Virtual microphone is active, returning true');
        return true;
      }
    }

    try {
      console.info('[GM MeetMind] [VirtualMic] Initializing virtual microphone');
      ...
      if (trackGenerator.id) {
        ...
        console.debug(`[GM MeetMind] [VirtualMic] Using existing track ID: ${trackGenerator.id}`);
      } else {
        // Try to set custom ID or properties if possible
        try {
          Object.defineProperty(trackGenerator, 'id', { value: VIRTUAL_MIC_ID });
        } catch (e) {
          console.debug('[GM MeetMind] [VirtualMic] Could not set custom track ID', e);
        }
      }
      ...
      try {
        ...
        console.debug('[GM MeetMind] [VirtualMic] Could not set track label', e);
      }
      
      isActive = true;
      audioTimestamp = performance.now() * 1000; // Reset timestamp to current time in microseconds
      
      console.info('[GM MeetMind] [VirtualMic] Virtual microphone initialized successfully');
      return true;
    } catch (error) {
      console.error('[GM MeetMind] [VirtualMic] Failed to initialize virtual microphone:', error);
      cleanup();
      return false;
    }
  }
  ...
  function addAudioData(pcmData, metadata = {}) {
    ...
    if (!pcmData || pcmData.length === 0) {
      console.warn('[GM MeetMind] [VirtualMic] Received empty audio data');
      return;
    }
    ...
    if (chunkIndex === undefined || totalChunks === undefined) {
      console.debug(`[GM MeetMind] [VirtualMic] Adding single audio chunk to ${immediate ? 'immediate' : 'regular'} playback queue (trackId: ${trackId})`);
      addBatchToQueue({
        data: floatData,
    ...
    console.debug(`[GM MeetMind] [VirtualMic] Received ${immediate ? 'immediate' : 'regular'} chunk ${chunkIndex + 1}/${totalChunks} for track ${trackId}`);
    ...
    if (buffer.chunks.size === buffer.totalChunks) {
      console.debug(`[GM MeetMind] [VirtualMic] Complete ${immediate ? 'immediate' : 'regular'} batch received for track ${trackId} (${buffer.totalChunks} chunks)`);
  ...
  function assembleCompleteBatch(trackId, immediate = false) {
    ...
    if (!buffer.has(trackId)) {
      console.error(`[GM MeetMind] [VirtualMic] Cannot assemble incomplete batch for track ${trackId} (buffer not found)`);
      return null;
    }
    
    const bufferData = buffer.get(trackId);
    if (!bufferData || bufferData.chunks.size < bufferData.totalChunks) {
      console.error(`[GM MeetMind] [VirtualMic] Cannot assemble incomplete batch for track ${trackId}`);
      return null;
    }
    ...
    for (let i = 0; i < bufferData.totalChunks; i++) {
      ...
      if (!chunk) {
        console.error(`[GM MeetMind] [VirtualMic] Missing chunk ${i} for track ${trackId}`);
        return null;
      }
    ...
    console.debug(`[GM MeetMind] [VirtualMic] Assembled batch: ${totalLength} samples for track ${trackId}`);
    return {
      data: combinedData,
  ...
  function addBatchToQueue(batch, immediate = false) {
    if (immediate) {
      ...
      console.debug(`[GM MeetMind] [VirtualMic] Added immediate batch to queue. Queue length: ${immediateQueue.length}`);
    } else {
      playbackQueue.push(batch);
      console.debug(`[GM MeetMind] [VirtualMic] Added batch to queue. Queue length: ${playbackQueue.length}`);
    }
  ...
  function startPlayback() {
    ...
    if (!isActive || !isWriterValid()) {
      if (!initializeVirtualMic()) {
        console.error('[GM MeetMind] [VirtualMic] Cannot start playback - virtual mic not ready');
        return;
      }
    }
    
    isPlaying = true;
    console.debug('[GM MeetMind] [VirtualMic] Starting playback process');
  ...
  async function processNextPlaybackBatch() {
    ...
    if (immediateQueue.length === 0 && playbackQueue.length === 0) {
      console.debug('[GM MeetMind] [VirtualMic] Playback queue empty, stopping playback');
      isPlaying = false;
      return;
    }
    ...
    try {
      ...
      if (!batchData) {
        console.debug('[GM MeetMind] [VirtualMic] No batches collected, stopping playback');
        isPlaying = false;
        return;
      }
      ...
      // console.debug(`[GM MeetMind] [VirtualMic] Audio analysis: ${silentSections.length} silent sections, ${silentPercentage.toFixed(2)}% silent`);
      ...
      //   console.warn(`[GM MeetMind] [VirtualMic] Found ${largeSilentSections.length} large silent sections`);
      ...
      //     console.warn(`[GM MeetMind] [VirtualMic] Silent section #${index + 1}: ${startTimeSeconds.toFixed(2)}s - ${endTimeSeconds.toFixed(2)}s (${durationSeconds.toFixed(2)}s long)`);
      //   });
      // }
      
      // if (isSilent) {
      //   console.warn(`[GM MeetMind] [VirtualMic] WARNING: Entire batch of ${durationSeconds.toFixed(2)}s is silent!`);
      // }
      
      // Play the combined batch
      const playbackDurationMs = await playAudioBatch(combinedBatch);
      
      console.debug(`[GM MeetMind] [VirtualMic] Played batch of ${combinedBatch.data.length} samples, duration: ${playbackDurationMs}ms`);
      ...
      if (!isStreamClosedError) {
        console.error('[GM MeetMind] [VirtualMic] Error in playback process:', error);
      }
  ...
  function collectBatchesForPlayback() {
    ...
    if (maxSamples >= MIN_BATCH_SIZE || !hasMoreData) {
      ...
      console.debug(`[GM MeetMind] [VirtualMic] Collected batches for mixing: immediate=${immediateBatches.length} (${immediateSamples} samples), regular=${regularBatches.length} (${regularSamples} samples), mixed=${maxSamples} samples, ${durationMs.toFixed(1)}ms`);
      ...
      console.debug(`[GM MeetMind] [VirtualMic] Not enough data for playback (${maxSamples} < ${MIN_BATCH_SIZE}), waiting for more`);
      return null;
    }
  }
  ...
  function mixAudioBatches(immediateBatches, regularBatches, maxSamples, targetSampleRate) {
    ...
    if (immediateAudio) {
      ...
      console.debug(`[GM MeetMind] [VirtualMic] Mixed immediate audio: ${immediateLength} samples`);
    }
    ...
    if (regularAudio) {
      ...
      console.debug(`[GM MeetMind] [VirtualMic] Mixed regular audio: ${regularLength} samples`);
    }
   ...
   */
  function cleanup() {
    console.info('[GM MeetMind] [VirtualMic] Cleaning up virtual microphone');
    ...
    if (audioWriter) {
      try {
        ...
        console.warn('[GM MeetMind] [VirtualMic] Error releasing writer lock:', error);
      }
      audioWriter = null;
    }
  ...
  function handleMessage(event) {
    ...
    if (type === 'PCM_DATA') {
      ...
      if (!pcmData || !Array.isArray(pcmData)) {
        console.error('[GM MeetMind] [VirtualMic] Invalid PCM data received');
        return;
      }
  ...
  function waitForDeviceEmulator() {
    return new Promise((resolve, reject) => {
      ...
      if (navigator.mediaDevices && typeof navigator.mediaDevices.addEmulatedDevice === 'function') {
        console.info('[GM MeetMind] [VirtualMic] Device emulator already available');
        resolve();
        return;
      }
      ...
      const handleDeviceEmulatorLoaded = () => {
        console.info('[GM MeetMind] [VirtualMic] Device emulator loaded event received');
  ...
  async function registerVirtualDevice() {
    if (virtualDeviceId) {
      console.debug('[GM MeetMind] [VirtualMic] Virtual device already registered');
      return;
    }
    ...
    try {
      ...
      console.info(`[GM MeetMind] [VirtualMic] Virtual microphone registered with device ID: ${virtualDeviceId}`);
      
    } catch (error) {
      console.error('[GM MeetMind] [VirtualMic] Failed to register virtual device:', error);
    }
  }
  ...
  window.addEventListener('dyte.deviceEmulatorLoaded', () => {
    console.info('[GM MeetMind] [VirtualMic] Device emulator loaded, registering virtual microphone...');
    registerVirtualDevice();
  });
  
  // Check if device emulator is already loaded
  if (navigator.mediaDevices && typeof navigator.mediaDevices.addEmulatedDevice === 'function') {
    console.info('[GM MeetMind] [VirtualMic] Device emulator already available, registering virtual microphone...');
    registerVirtualDevice();
  }
  ...
  console.info('[GM MeetMind] [VirtualMic] Virtual microphone setup complete (Device Emulator version)');
})();

...
export class SettingsService implements ISettingsService {
  ...
  constructor() {
    ...
    console.info(`[GM MeetMind] [SettingsService] Using ${this.usesChromeStorage ? 'Chrome Storage' : 'localStorage'} for settings persistence`);
  }
  ...
  async getSetting<T>(key: string, defaultValue: T): Promise<T> {
    try {
        ...
        return new Promise<T>((resolve) => {
          ...
          chrome.storage.sync.get(key, (result: Record<string, any>) => {
            ...
            if (chrome.runtime.lastError) {
              // @ts-ignore - Chrome API is defined in global scope for extensions
              console.error(`[GM MeetMind] [SettingsService] Error getting setting ${key}:`, chrome.runtime.lastError);
      ...
      console.error(`[GM MeetMind] [SettingsService] Error getting setting ${key}:`, error);
      return defaultValue;
    }
  }
  ...
  async loadAllSettings<T extends object>(defaultSettings: T): Promise<T> {
    try {
      if (this.usesChromeStorage) {
        ...
        return new Promise<T>((resolve) => {
          ...
          chrome.storage.sync.get(keys, (result: Record<string, any>) => {
            ...
            if (chrome.runtime.lastError) {
              // @ts-ignore - Chrome API is defined in global scope for extensions
              console.error('[GM MeetMind] [SettingsService] Error loading all settings:', chrome.runtime.lastError);
      ...
      console.error('[GM MeetMind] [SettingsService] Error loading all settings:', error);
  ...
  async validateApiKeyAndFetchModels(
    ...
    try {
      ...
      console.error(`[GM MeetMind] [SettingsService] Error validating API key and fetching models for ${provider}:`, error);
      return {
        validation: {
          valid: false,
